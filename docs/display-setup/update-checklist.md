...
- Laptop-only: disconnect external displays, reload kanshi, confirm `eDP-1` is enabled, DPMS is on, workspaces `1` and `2` are on it, and `kanshictl status` reports `laptop`.
...
- Docked lid-open: while USB-C monitor is still connected, open the lid and confirm both `DP-1` and `eDP-1` are enabled, workspace `1` is on `DP-1`, workspace `2` is on `eDP-1`, and `kanshictl status` reports a docked-open profile.
...
Expected result: both `DP-1` and `eDP-1` are enabled, `eDP-1` has DPMS on, workspace `1` is on `DP-1`, workspace `2` is on `eDP-1`, and kanshi reports `docked_open_dp_only` or `docked_open_dp_hdmi`.
...
Expected result: `eDP-1` is enabled with DPMS on, workspace `1` and `2` are on `eDP-1`, and kanshi reports `laptop`.
