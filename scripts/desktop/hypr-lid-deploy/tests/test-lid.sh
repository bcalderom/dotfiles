...
case "${1:-}" in
  ...
  eval)
    code="${2:-}"
    if [[ "${code}" == *"hl.monitor("* ]]; then
      monitor="${code#*output = \'}"; monitor="${monitor%%\'*}"
      if [[ "${code}" == *"disabled = true"* ]]; then set_monitor_state "${monitor}" 0; else maybe_skip_monitor_enable "${monitor}" && { printf 'ok\n'; exit 0; }; set_monitor_state "${monitor}" 1; fi
    fi
    printf 'ok\n'
    ;;
  keyword) printf "keyword can't work with non-legacy parsers. Use eval.\n" ;;
  dispatch)
    case "${2:-}" in hl.dsp.workspace.move*|hl.dsp.focus*) printf 'ok\n' ;; *) printf 'error: invalid dispatcher\n'; exit 7 ;; esac
    ;;
  reload)
...
assert_contains "hl.workspace_rule({ workspace = '1', monitor = 'DP-1', persistent = false })"
assert_contains "hl.workspace_rule({ workspace = '2', monitor = 'DP-1', persistent = false })"
assert_contains "hl.workspace_rule({ workspace = '10', monitor = 'DP-1', persistent = false })"
assert_not_contains "hl.dsp.workspace.move({ workspace = '2', monitor = 'DP-1' })"
assert_not_contains "hl.dsp.workspace.move({ workspace = '3', monitor = 'DP-1' })"
assert_contains "hl.workspace_rule({ workspace = '3', monitor = 'DP-1', persistent = false })"
assert_contains "hl.monitor({ output = 'eDP-1', disabled = true })"
assert_before "hl.dsp.workspace.move({ workspace = '1', monitor = 'DP-1' })" "hl.monitor({ output = 'eDP-1', disabled = true })"
assert_not_contains "hl.dsp.dpms"
...
assert_contains "hl.monitor({ output = 'DP-1', mode = '2560x1440@120.01', position = '1920x0', scale = 1, disabled = false })"
...
assert_not_contains "hl.dsp.workspace.move"
...
assert_contains "hl.monitor({ output = 'eDP-1', mode = 'preferred', position = '0x0', scale = 1, disabled = false })"
assert_not_contains "hl.dsp.dpms"
assert_contains "hl.workspace_rule({ workspace = '3', monitor = 'eDP-1', persistent = false })"
assert_contains "hl.dsp.workspace.move({ workspace = '3', monitor = 'eDP-1' })"
assert_contains "hl.dsp.focus({ workspace = '2' })"
assert_not_contains "hl.monitor({ output = 'DP-1', mode = '2560x1440"
assert_no_process_restart
[[ "$(cat "${BACKLIGHT_VALUE_PATH}")" -eq 100 ]]
[[ ! -e "${HYPR_LID_BACKLIGHT_STATE_FILE}" ]]
echo "==> migrate laptop workspaces when the dock connects"
reset_log
set_monitors "DP-1 eDP-1" "DP-1 eDP-1"
HYPR_WORKSPACES=internal12 HYPR_ACTIVE_WS=2 HYPR_ACTIVE_MONITOR=eDP-1 run_lid open
assert_contains "hl.dsp.workspace.move({ workspace = '1', monitor = 'DP-1' })"
assert_contains "hl.dsp.workspace.move({ workspace = '2', monitor = 'DP-1' })"
...
assert_contains "hl.workspace_rule({ workspace = '4', monitor = 'eDP-1', persistent = false })"
assert_contains "hl.dsp.workspace.move({ workspace = '4', monitor = 'eDP-1' })"
assert_contains "hl.dsp.focus({ workspace = '3' })"
...
assert_contains "hl.workspace_rule({ workspace = '3', monitor = 'eDP-1', persistent = false })"
assert_contains "hl.workspace_rule({ workspace = '4', monitor = 'DP-1', persistent = false })"
assert_not_contains "hl.dsp.focus({ workspace = '4' })"
...
assert_not_contains "hl.dsp.dpms"
assert_contains "hl.workspace_rule({ workspace = '1', monitor = 'eDP-1'"
assert_not_contains "hl.workspace_rule({ workspace = '3', monitor = 'eDP-1', persistent = true })"
assert_contains "hl.workspace_rule({ workspace = '4', monitor = 'eDP-1', persistent = false })"
assert_contains "hl.dsp.focus({ workspace = '2' })"
assert_not_contains "hl.dsp.focus({ workspace = '4' })"
...
assert_before "hl.monitor({ output = 'eDP-1', mode = 'preferred'" "reload"
assert_contains "hl.workspace_rule({ workspace = '1', monitor = 'eDP-1', persistent = false })"
...
assert_contains "hl.monitor({ output = 'eDP-1', mode = 'preferred', position = '0x0', scale = 1, disabled = false })"
assert_contains "hl.monitor({ output = 'HDMI-A-1', mode = '1920x1080@60', position = '0x0', scale = 1, mirror = 'eDP-1', disabled = false })"
assert_before "hl.monitor({ output = 'eDP-1', mode = 'preferred'" "hl.monitor({ output = 'HDMI-A-1', mode = '1920x1080@60'"
assert_contains "hl.dsp.focus({ workspace = '2' })"

if grep -Fq 'kanshi' "${LID_SCRIPT}"; then
  if grep -Eq 'kanshictl|hl\.monitor.*kanshi' "${LID_SCRIPT}"; then
